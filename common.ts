...
import { readableStreamToString, IrisSoftwareGroupCascadeError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://api.iris.co.uk/oauth2/v1/token"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new IrisSoftwareGroupCascadeError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new IrisSoftwareGroupCascadeError(e, e.response?.data)
                }
                if (innerError instanceof IrisSoftwareGroupCascadeError) {
                    // Got "IrisSoftwareGroupCascadeError" from the above try block
                    throw innerError;
                }
